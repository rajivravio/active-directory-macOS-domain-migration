...
#	  Hostname needs to be <15 characters - Add feature to check compliance?
...
banner=`/Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType hud -title "Domain migration" -heading "QATAR to ADS" -description "We are moving your user account from QATAR to ADS.  When we are completed, and your computer restarts, you will be able to login to your computer with your new domain credentials. Please contact helpdesk@qatar.northwestern.edu for assitance if you have trouble logging in" -button1 "Proceed" -button2 "Not Now" -defaultButton 1 -cancelButton 2 -timeout 180 -countdown` 
...
if [[ "$currentAD" = "$oldAD" ]]; then
    ...
    # graceful unbind attempt
    dsconfigad -remove $oldAD -username <un> -password <pw>
    
    # force the client to sever ties (if the line above fails, the machine will still be unbound but the dc will have a zombie device obj)
